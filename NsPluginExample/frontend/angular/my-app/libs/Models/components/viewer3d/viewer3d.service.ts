...
export class Viewer3dService {
  ...
  get browserSupport(): boolean {
    return (
      this.window.nativeWindow.ActiveXObject !== undefined ||
      'ActiveXObject' in this.window.nativeWindow
    );
  }
  ...
  private destruct(): void {
    ...
    if (this.browserSupport) {
      const objects = this.window.nativeWindow.document.querySelectorAll(
        'object'
      );
      if (objects) {
        objects.forEach(x => {
      ...
      if (typeof this.window.nativeWindow.CollectGarbage === 'function') {
        this.window.nativeWindow.CollectGarbage();
      }
    }
  }
   ...
   */
  getPluginInstance(
    pluginOptions: IP3dbPluginOptions,
    index: number,
    parent: HTMLElement
  ): P3dbPluginInstance {
  ...
  private createPluginInstance(
    ...
    const pluginElement = this.window.nativeWindow.document.createElement(
      'object'
    );
    pluginElement.setAttribute(
      'classid',
      'clsid:13139E6A-CBA5-471C-A0BA-C05860EB3F1A'
    );
    pluginElement.setAttribute('id', this.getPluginId(index));
    pluginElement.hspace = 0;
    pluginElement.vspace = 0;
    pluginElement.border = '0';
    pluginElement.appendChild(
      this.createParamElement('licenceurl', pluginOptions.LicenceUrl)
    );
    pluginElement.appendChild(
      this.createParamElement('debug', pluginOptions.Debug.toString())
    );
    pluginElement.appendChild(
      this.createParamElement('version', pluginOptions.Version)
    );
    pluginElement.appendChild(
      this.createParamElement(
        'contributionCullingThreshold',
        pluginOptions.ContributionCullingThreshold.toString()
      )
    );
    ...
    createDiv.appendChild(
      this.createScriptElement('InitializationComplete', index)
    );
    createDiv.appendChild(
      this.createScriptElement('ContextMenuCommandExecuted', index)
    );
    createDiv.appendChild(
      this.createScriptElement('ModelLoadingStateChanged', index)
    );
