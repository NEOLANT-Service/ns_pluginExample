...
import { IOpenFileDialogResult } from 'libs/Models/types/p3dbplugin.type';
...
export class ModelsComponent implements OnInit, OnDestroy {
  ...
  commandClickHandler(event: IToolboxClickEvent) {
    let showDialogResult: IOpenFileDialogResult[];
    ...
    switch (event.button.name) {
      ...
      case 'openFiles':
        ...
        if (showDialogResult && showDialogResult.length > 0) {
          this.viewer3d.loadFilesAsync(showDialogResult, LoadFileMode.Load);
        }
        break;
      case 'addFiles':
        showDialogResult = this.viewer3d.showFileDialog();
        if (showDialogResult && showDialogResult.length > 0) {
          this.viewer3d.loadFilesAsync(showDialogResult, LoadFileMode.Add);
        }
        break;
      case 'removeFiles':
